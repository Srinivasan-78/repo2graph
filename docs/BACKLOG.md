...
| **No per-file tree-sitter parse timeout** *(Shipped)* | M | **Shipped:** `PARSE_TIMEOUT_MICROS` (5s) bounds `parser.parse` in `parse_source`. Native `timeout_micros`/`set_timeout_micros` is used when the binding has it; tree-sitter 0.26 + language-pack 1.20 do not (progress_callback is ignored for bytes and segfaults for a reader), so the fallback is a chunked reader that raises on the wall clock. Fixtures in `tests/test_parse_timeout.py`. |
