...
jobs:
  impact:
    ...
    steps:
      - name: "Harden Runner"
        uses: step-security/harden-runner@4d991eb9b905fa189e4c376166672c3f2f2378fb # v2.11.0
        with:
          egress-policy: audit
      ...
      - name: "Run repo2graph Impact Analysis"
        ...
        run: |
          ...
          repo2graph impact -i .repo2graph-base --base "origin/${{ github.base_ref }}" --head "HEAD" --format markdown --write .impact-reports/impact-report.md
          repo2graph impact -i .repo2graph-base --base "origin/${{ github.base_ref }}" --head "HEAD" --format json --write .impact-reports/impact-report.json
          repo2graph impact -i .repo2graph-base --base "origin/${{ github.base_ref }}" --head "HEAD" --format sarif --write .impact-reports/impact-report.sarif
          repo2graph impact -i .repo2graph-base --base "origin/${{ github.base_ref }}" --head "HEAD" --format pr-comment --write .impact-reports/pr-comment.md
      ...
      - name: "Upload SARIF Results to GitHub Code Scanning"
        uses: github/codeql-action/upload-sarif@6bb034416e7612943e8a45de8880f4b8564e130e # v3.28.11
